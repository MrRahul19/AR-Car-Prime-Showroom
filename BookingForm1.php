...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $car_variant = $conn->real_escape_string($_POST['car_variant']);
    ...
    // Prepare SQL query
    $sql = "INSERT INTO car_bookingform1 (car_model, car_variant, car_color, booking_date, delivery_date, delivery_time, manufacture_year, car_type, insurance_type, insurance_company, transmission, standard_accessories, optional_accessories, technological_addons, protective_accessories, luxury_addons, loan_option, loan_amount, loan_tenure, car_loan_provider, comments) 
            VALUES ('$car_model', '$car_variant', '$car_color', '$booking_date', '$delivery_date', '$delivery_time', '$manufacture_year', '$car_type', '$insurance_type', '$insurance_company', '$transmission', '$standard_accessories', '$optional_accessories', '$technological_addons', '$protective_accessories', '$luxury_addons', '$loan_option', '$loan_amount', '$loan_tenure','$car_loan_provider', '$comments')";
    ...
    <div class="main-form">
        <div class="form">
            ...
            <form action="BookingForm1.php" method="POST">
                ...
                <input type="text" id="car_variant" name="car_variant" required><br><br>
                ...
                <input type="time" id="delivery_time" name="delivery_time" required><br><br>

                <label for="manufacture_year">Manufacture Year (Model of):</label>
                <input type="number" id="manufacture_year" name="manufacture_year" min="2015" max="2025" required><br>
                ...
                <div class="loan-option-form">
                    ...
                    <div id="loan_details" class="loan-details">
                        ...
                        <button type="button" onclick="calculateEMI()" disabled>Calculate EMI</button>
...
<script>
// Toggle visibility of loan details based on loan selection
document.getElementById('loan').addEventListener('change', function() {
    ...
    const calculateButton = document.querySelector('button[type="button"]'); // EMI button
    const carLoanProviderField = document.getElementById('car_loan_provider');

    if (this.value === 'Yes') {
        loanDetails.classList.add('active');  // Show the loan details
        loanAmountField.disabled = false; // Enable loan amount
        loanTenureField.disabled = false; // Enable loan tenure
        calculateButton.disabled = false; // Enable EMI calculation button
        carLoanProviderField.disabled = false; // Enable car loan provider field (if visible)
    } else {
        loanDetails.classList.remove('active');  // Hide the loan details
        loanAmountField.disabled = true; // Disable loan amount
        loanTenureField.disabled = true; // Disable loan tenure
        calculateButton.disabled = true; // Disable EMI calculation button
        carLoanProviderField.disabled = true; // Disable car loan provider field
    }
});
    ...
    function calculateEMI() {
        ...
        const loanTenure = parseInt(document.getElementById('loan_tenure').value);
        const interestRate = 9; // Fixed at 9%
        ...
        if (loanAmount && loanTenure) {
            ...
            document.getElementById('emi_result').innerHTML = `Your EMI is ₹${emiFormatted} per month.`;
        } else {
            document.getElementById('emi_result').innerHTML = 'Please enter valid details for EMI calculation.';
        }
    }
    //this alert msg show before the from filling time 
    window.onload = function() {
        alert('This is the -car details form- fill the correct & accurate information');
    };
</script>
