...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $totalCarPrice = $_POST['total_car_price'] ?? '';
    $finalAmountPayable = $_POST['final_amount_payable'] ?? '';
    $youSavingAmount = $_POST['you_saving_amount'] ?? '';
    $loanOption = $_POST['loan'] ?? ''; // Corrected to match the input name
    ...
    $sql = "INSERT INTO car_bookingform3 (car_model, total_car_price, final_amount_payable, you_saving_amount, loan_option, payment_date, payment_time, payer_name, comments)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Prepare the statement
    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters
        $stmt->bind_param("sdddsdsss", $carModel, $totalCarPrice, $finalAmountPayable, $youSavingAmount, $loanOption, $paymentDate, $paymentTime, $payerName, $comments);
                ...
                <form action="BookingForm3.php" method="POST">
                    ...
                    <!-- Payment Method Dropdown -->
                        ...
                        <div id="qr_section" style="display:flex; justify-content: center; align-items: center; background-color: #f0f0f0; height: 300px; width: 100%; border-radius: 10px; margin-top: 10px;">
                            ...
                            <img src="Cars-IMG/QR.jpg" alt="AR Prime Showroom QR Code" style="width: 380px; height: 295px;" readonly>
                        </div><br>
